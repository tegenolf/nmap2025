...
class Machine(object):
    ...
    def __init__(self, gamma_ref: float = gamma_ref, circumference: float = circumference,
                 voltage: float = voltage, harmonic: int = harmonic,
                 alpha_c: float = alpha_c, phi_s: float = phi_s) -> None:
    ...
    def eta(self, deltap: np.ndarray | float) -> np.ndarray | float:
    ...
    def update_gamma_ref(self) -> None:
    ...
    def get_gamma_ref(self) -> float:
        return self.gamma_ref
		
    def get_alpha_c(self) -> float:
        return self.alpha_c
		
    def set_alpha_c(self, alpha_c: float) -> None:
        self.alpha_c = alpha_c

def track_one_turn(z_n: np.ndarray | float, deltap_n : np.ndarray | float, machine : Machine) -> tuple():
    m = machine
    # half drift
...
def T(deltap: np.ndarray, machine: Machine) -> np.ndarray:
    '''Kinetic energy term in Hamiltonian.'''
    return -0.5 * machine.eta(deltap) / machine.p0() * deltap**2

def U(z: np.ndarray, machine: Machine, beta_: float = None) -> np.ndarray:
...
def hamiltonian(z: np.ndarray, deltap: np.ndarray, machine: Machine) -> np.ndarray:
    return T(deltap, machine) + U(z, machine, beta_=beta(gamma(machine.p0() + deltap)))

def plot_hamiltonian(machine: Machine, zleft: float = -50, zright: float = 50, dpmax: float = 0.01, cbar: bool = True) -> None:
...
def plot_hamiltonian_ax(machine: Machine, ax: matplotlib.axes._axes.Axes, zleft: float = -50, zright: float = 50, dpmax: float = 0.01) -> None:
    '''Plot Hamiltonian contours across (zleft, zright) and (-dpmax, dpmax).'''
    Z, DP = np.meshgrid(np.linspace(zleft, zright, num=1000),
                        np.linspace(-dpmax, dpmax, num=1000))
    H = hamiltonian(Z, DP * machine.p0(), machine) / machine.p0()
    
    ax.contourf(Z, DP, H, cmap=plt.get_cmap('hot_r'), levels=12,
                 zorder=0, alpha=0.5)
    ax.set_xlabel('$z$ [m]')
    ax.set_ylabel(r'$\delta$')
    
    ax.contour(Z, DP, H, colors='b', linewidths=2, levels=[0])
    
def plot_rf_overview(machine: Machine) -> tuple():
    m = machine
...
def emittance(z: np.ndarray, deltap: np.ndarray) -> float:
    N = len(z)
